...
import { Calendar, Mail, MapPin, Link as LinkIcon, Download, ArrowRight, X, Send, CheckCircle2, Loader2, Clock, CheckCircle } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-stone-50 font-sans text-stone-900 selection:bg-blue-100 selection:text-blue-900">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-stone-200/50 transition-all">
        <div className="max-w-4xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <button 
            ...
            className="bg-stone-900 text-white px-6 py-2.5 rounded-xl text-sm font-bold hover:bg-stone-800 transition-all hover:shadow-lg active:scale-95"
          >
            문의하기
          </button>
        </div>
      ...
      <main className="max-w-4xl mx-auto px-6 pt-32 pb-20 space-y-24">
        ...
        {/* 2. Profile Intro (Magazine Style) */}
        <section className="flex flex-col md:flex-row gap-12 items-center md:items-start animate-in fade-in slide-in-from-bottom-8 duration-700">
           ...
           <div className="flex-1 text-center md:text-left space-y-6">
             <div>
                <h1 className="text-4xl md:text-5xl font-extrabold text-stone-900 mb-3 tracking-tight leading-tight">
                    {user.name}
                </h1>
             ...
             <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-2">
               {user.tags.map(tag => (
                 <span key={tag} className="px-4 py-1.5 bg-white border border-stone-200 text-stone-600 rounded-full text-sm font-semibold shadow-sm hover:border-blue-300 hover:text-blue-600 transition-colors cursor-default">
                   {tag}
                 </span>
               ))}
             </div>
             ...
             <div className="flex justify-center md:justify-start gap-6 pt-2 text-sm font-bold text-stone-400">
                <a href="#" className="flex items-center gap-2 hover:text-blue-600 transition-colors group">
                    <div className="p-2 bg-white rounded-full border border-stone-200 group-hover:border-blue-200 group-hover:bg-blue-50">
                        <LinkIcon size={16}/> 
                    </div>
                    website.com
                </a>
                ...
                <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-lg relative z-10 overflow-hidden animate-in fade-in zoom-in duration-300">
                    ...
                    {inquiryStatus === 'success' ? (
                        <div className="p-16 flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <p className="text-stone-500 font-medium mb-8">빠른 시일 내에 이메일로 답변 드리겠습니다.</p>
                            <button onClick={resetInquiry} className="bg-stone-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-stone-800 transition-colors">
                                확인
                            </button>
                        </div>
                    ) : (
                        <>
                            ...
                            <div className="p-8 space-y-6">
                                ...
                                <button 
                                    ...
                                    className="w-full bg-stone-900 text-white font-bold py-4 rounded-xl hover:bg-stone-800 transition-all shadow-lg shadow-stone-900/20 active:scale-[0.98] flex items-center justify-center gap-2"
                                >
                     ...
                     <div className="flex justify-between items-start mb-6">
                         <div>
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider mb-3 ${selectedProject.status === 'in-progress' ? 'bg-blue-100 text-blue-700' : selectedProject.status === 'completed' ? 'bg-green-100 text-green-700' : 'bg-stone-100 text-stone-600'}`}>
                                {selectedProject.status === 'in-progress' && <Clock size={12} />}
                                {selectedProject.status === 'completed' && <CheckCircle size={12} />}
                                {selectedProject.status === 'planning' && '계획 중'}
                                {selectedProject.status === 'in-progress' && '진행 중'}
                                {selectedProject.status === 'completed' && '완료됨'}
                            </span>
                            <h3 className="text-2xl font-extrabold text-stone-900 leading-tight">{selectedProject.title}</h3>
                         </div>
                         <button onClick={() => setSelectedProject(null)} className="p-2 bg-stone-50 rounded-full text-stone-400 hover:bg-stone-100 hover:text-stone-900"><X size={20} /></button>
                     </div>

                     <div className="space-y-4">
                        <div className="flex gap-4 p-4 bg-stone-50 rounded-2xl border border-stone-100">
                             <div>
                                 <div className="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1">클라이언트</div>
                                 <div className="font-bold text-stone-900 text-sm">{selectedProject.client}</div>
                             </div>
                             <div className="w-px bg-stone-200"></div>
                             <div>
                                 <div className="text-[10px] font-bold text-stone-400 uppercase tracking-wider mb-1">기간</div>
                                 <div className="font-bold text-stone-900 text-sm">{selectedProject.startDate} ~ {selectedProject.endDate}</div>
                             </div>
                        </div>
                        
                        <div>
                            <div className="text-xs font-bold text-stone-500 uppercase mb-2">프로젝트 설명</div>
                            <p className="text-stone-600 text-sm leading-relaxed">
                                {selectedProject.description || '상세 설명이 없습니다.'}
                            </p>
                        </div>
                     </div>
                </div>
